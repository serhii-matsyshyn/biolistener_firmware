...
{
  uint16_t status;
  flex32_t ch[ADS131M08_NUM_CHANNELS];
};

enum ADS131M08_ChannelState
{
  DISABLE,
  ENABLE
};
...
public:
  ...
  void begin(uint8_t cs_pin, uint8_t drdy_pin, uint8_t reset_pin, SPIClass &spi_ads131m08);
  void begin(uint8_t sclk, uint8_t miso, uint8_t mosi, uint8_t cs_pin, uint8_t drdy_pin, uint8_t reset_pin, SPIClass &spi_ads131m08);
...
private:
  ...
  uint8_t drdyPin;
  uint8_t resetPin;
  ADS131M08_PgaGain pgaGain[ADS131M08_NUM_CHANNELS];
  AdcOutput fullScale;
